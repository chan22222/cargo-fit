...
const App: React.FC = () => {
  ...
  const findBestPositionBottomFirst = (
    ...
    for (const baseY of yLevels) {
      for (const { x, z } of xzPoints) {
        ...
        // 이 XZ 위치에서 실제로 놓일 Y 계산 (겹치는 아이템 위로)
        let maxY = baseY;
        for (const item of existingItems) {
  ...
  const findBestPositionInnerFirst = (
    ...
    const xzPoints: { x: number, z: number }[] = [
      { x: 0, z: startZ },
      { x: container.width - dims.width, z: startZ },
      { x: 0, z: 0 }
    ];
  ...
  // 현재 사용 중인 최대 컨테이너 인덱스 계산
  const getMaxContainerIndex = (items: PackedItem[]) => {
    if (items.length === 0) return 0;
    return Math.max(...items.map(item => item.containerIndex ?? 0));
  };

  const handleAddCargo = (newItem: Omit<CargoItem, 'id'> | CargoItem) => {
    const currentContainer = CONTAINER_SPECS[containerType];
    const maxContainers = 10; // 최대 컨테이너 수

    // 다중 컨테이너 배치 함수 - 항상 0번부터 빈 공간 탐색
    const placeItemWithMultiContainer = (
      item: CargoItem,
      currentPackedItems: PackedItem[],
      uniqueIdPrefix: string,
      startIndex: number
    ): { placed: boolean; items: PackedItem[]; containerIndex: number } => {
      // 같은 그룹의 이미 배치된 아이템 찾기 (회전 방향 통일용)
      ...
      for (let tryContainer = 0; tryContainer < maxContainers; tryContainer++) {
        const containerItems = getContainerItems(currentPackedItems, tryContainer);
        let bestPosition = null;
        ...
        if (bestPosition) {
          const newPackedItem: PackedItem = {
            ...item,
            uniqueId: `${uniqueIdPrefix}-${startIndex}-${Date.now()}`,
          ...
          return {
            placed: true,
            items: [...currentPackedItems, newPackedItem],
            containerIndex: tryContainer
          };
        }
      }

      return { placed: false, items: currentPackedItems, containerIndex: -1 };
    };
  ...
  const getSEOConfig = () => {
    const seoConfigs: Record<string, { title?: string; description: string; keywords: string; canonicalUrl: string; jsonLd: object[] }> = {
      ...
      'tracker/air': {
        title: '항공화물 추적',
        description: '항공화물 실시간 추적 서비스. AWB 번호로 전 세계 항공화물 위치 확인.',
        keywords: '항공화물 추적, AWB 추적, 항공운송, 화물 위치',
        canonicalUrl: `${BASE_URL}/tracker/air`,
        jsonLd: [schemas.softwareApplication('항공화물 추적', '항공화물 실시간 추적 서비스.', `${BASE_URL}/tracker/air`)],
      },
      'tracker/courier': {
        title: '택배 추적',
        description: '국내외 택배사 통합 추적. 운송장 번호로 배송 상태 실시간 확인.',
        keywords: '택배 추적, 배송 조회, 운송장 조회, 국제택배',
        canonicalUrl: `${BASE_URL}/tracker/courier`,
        jsonLd: [schemas.softwareApplication('택배 추적', '국내외 택배사 통합 추적 서비스.', `${BASE_URL}/tracker/courier`)],
      },
      'tracker/post': {
        title: '우편물 추적',
        description: '국제우편, EMS, 등기우편 추적 서비스. 전 세계 우편물 배송 상태 확인.',
        keywords: '우편 추적, EMS 추적, 등기 추적, 국제우편',
        canonicalUrl: `${BASE_URL}/tracker/post`,
        jsonLd: [schemas.softwareApplication('우편물 추적', '국제우편, EMS, 등기우편 추적 서비스.', `${BASE_URL}/tracker/post`)],
      },
      'tracker/rail': {
        title: '철도화물 추적',
        description: '철도화물 운송 추적 서비스. 화차 번호로 철송 화물 위치 확인.',
        keywords: '철도화물 추적, 철송 추적, 화차 추적, 육상운송',
        canonicalUrl: `${BASE_URL}/tracker/rail`,
        jsonLd: [schemas.softwareApplication('철도화물 추적', '철도화물 운송 추적 서비스.', `${BASE_URL}/tracker/rail`)],
      },
      fssc: {
        title: '항공 유류할증료/보안료 조회',
        description: '항공사별 유류할증료(FSC)와 보안료(SCC) 실시간 조회. 항공화물 운임 계산에 필수.',
        keywords: '유류할증료, 보안료, FSC, SCC, 항공화물, 항공운임, Fuel Surcharge, Security Charge',
        canonicalUrl: `${BASE_URL}/fssc`,
        jsonLd: [schemas.softwareApplication('항공 유류할증료/보안료 조회', '항공사별 유류할증료(FSC)와 보안료(SCC) 실시간 조회.', `${BASE_URL}/fssc`)],
      },
      worldclock: {
        title: '세계 시간',
  ...
  return (
    <>
      ...
      <div className="min-h-screen w-full bg-white flex flex-col font-sans text-slate-900" style={{ overflowX: 'clip' }}>
